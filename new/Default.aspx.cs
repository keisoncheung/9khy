...
public partial class new_Default : System.Web.UI.Page
{
    ...
    public string tejiazhuanqu(int par)
    {
        ...
        StringBuilder sbr = new StringBuilder();
        bool bol = false;
        DataTable dt = OleDbHelper.ExecuteDataSet(OleDbHelper.Conn, "select top 10 id,parid, Title,linkpic,picture from news where parid=26 order by id desc", null).Tables[0];
        string title = "";
        //sbr.Append("<table><tr>");
        foreach (DataRow dr in dt.Select())
        {
            ...
            sbr.Append("<li><div class='pic'><a href='../news.aspx?key=" + par + "&nid=" + dr["id"] + "' title='" + dr["Title"] + "'><img src='../pic/" + dr["picture"] + "' onerror='../pic/nopic.jpg'  /></a></div><div class='title'><a href='../news.aspx?key=" + par + "&nid=" + dr["id"] + "' title='" + dr["Title"] + "'>" + title + "</a></div></li>");
    ...
    public string gethotel()
    {
        StringBuilder sbr = new StringBuilder();
        int i = 0;
        string sql = "select top 15 id ,proname,canshu,Pictureb from product where parid in(select id from productType where parid=331) and grade=1 order by txtsortid asc,id desc";
        string canshu = "";
        DataTable dt = OleDbHelper.ExecuteDataSet(OleDbHelper.Conn, sql, null).Tables[0];
        foreach (DataRow dr in dt.Rows)
        {
            canshu = Convert.ToString(dr["canshu"]);
            if (canshu.Length > 38)
                canshu = canshu.Substring(0, 38) + "...";
            if (i < 3)
            {
                sbr.Append("<div class='khotel_index2div'><div class='khotel_index_imgdiv2'><a href='../hotelshow.aspx?pid=" + dr["id"] + @"'><img src='../pic/" + dr["Pictureb"] + @"' /></a></div><div style='width:250px; font-weight:bold;text-align:center;'><font style='color:#A40000;'>" + dr["proname"] + "</font></div><div class='khotel_index_condiv2'> " +canshu + @"</div></div>");
            }
            else
            {
                sbr.Append(@"<dl class=''>
...
<div class='kcd_index_condiv2'>
    ...
    public string frn_link()
    {
        ...
        foreach (DataRow dr in dt.Select())
        {
            sbr.Append(" <a href='" + dr[2] + "' target='_blank'><font style='color:white;'>" + dr[1] + "</a>&nbsp;|&nbsp;</font>");
        }
